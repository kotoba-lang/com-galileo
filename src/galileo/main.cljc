(ns galileo.main
  "Kotodama WASM entrypoint for the Galileo clean-room actor (L4) — Clojure port.
  ...
  facts (`galileo.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "galileo")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"galileo.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "galileo-compat" :tier tier :entities entities} 200])
...
(defn start! [] :galileo-compat/ready)
